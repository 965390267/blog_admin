  ...
  <div id="edit">
    ...
    <div class="efitwrap">
      <div class="article-left">
        <div>
          <input type="text" placeholder="请在此处输入文章标题" v-model="title" />
        </div>
        <div>
          <input type="text" placeholder="作者" v-model="author" />
        </div>
        <div>
          <input type="date" placeholder="时间" v-model="time" />
        </div>
      ...
      <div class="article-right-aside">
        <div class="col-md-3">
          ...
          <div class="add-article-box">
            ...
            <div class="add-article-box-content">
              <select @change="choosecolumn($event)">
                <option>技术</option>
                <option class="column">个人</option>
              </select>
            </div>
          </div>
          <div class="add-article-box">
  ...
  mounted() {
    ...
    editor.customConfig.uploadImgHooks = {
      success: function(xhr, editor, result) {
        ...
        that.imgsrc = `/articleimg/${result.data.imgname}`;
      },
      customInsert: function(insertImg, result, editor) {
        // 图片上传并返回结果，自定义插入图片的事件（而不是编辑器自动插入图片！！！）
        // insertImg 是插入图片的函数，editor 是编辑器对象，result 是服务器端返回的结果

        var url = `/articleimg/${result.data.imgname}`;
        insertImg(url);
